  ...
  browser.storage.local.get('tasks', function(obj) {
    ...
    if (items.length > 0) {
      ...
      for (var i = 0; i < items.length; i++) {
        todoList +=
          "<li class='task-item-div'><input class='task-checkbox' type='checkbox' tabindex='-1'><span class='task-item'>" +
          items[i] +
          '</span></li>';
      }
    ...
    if (
      ...
      browser.storage.local.get('tasks', function(obj) {
        ...
        browser.storage.local.set({ tasks: tasks }, function() {
          var taskItemDiv =
            "<li class='task-item-div'><input class='task-checkbox' type='checkbox'><span class='task-item'>" +
            task +
            '</span></li>';
